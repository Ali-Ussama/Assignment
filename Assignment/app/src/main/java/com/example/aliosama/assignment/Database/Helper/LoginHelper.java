package com.example.aliosama.assignment.Database.Helper;
...
import com.example.aliosama.assignment.Database.Models.*;
...
public class LoginHelper extends SQLiteOpenHelper {
    ...
    // Student Course
    private static final String Student_Course_Table = "Student_Course";

    Context context;
    static StudentModel studentModel;
    static TeacherModel teacherModel;
    static CourseModel mCourseModel;
    static AssignmentModel mAssignmentModel;
    ...
    public void onCreate(SQLiteDatabase db) {
        try {
            db.execSQL("create table "+StudentTable+" ( "+id+" integer primary key autoincrement , "+
                    name+" text , "+email+" text , "+password+" text, "+department+" text, "+section+" text, "+
                    student_image+" blob ); ");
            ...
            db.execSQL("create table "+CourseTable+" ( "+ id + " integer primary key autoincrement , "
                    ...
                    + semester + " text , "+description+" text, "+ teacher_id + " integer ); ");
            ...
            db.execSQL("create table "+Student_Course_Table+" ( "+id+" integer primary key autoincrement , "+
    ...
    public void AddStudentForCourse(int StudentID,int CourseID){
        try {
            ...
            db.insert(Student_Course_Table,null,contentValues);
        }catch (Exception e){
            e.printStackTrace();
        }
    }
    public ArrayList<StudentModel> getStudentsForCourse(int CourseID){
        ArrayList<StudentModel> data = new ArrayList<>();
        ArrayList<Integer> students_ids = new ArrayList<>();
        SQLiteDatabase db = getReadableDatabase();
        String Sql = "select "+student_id+" from "+Student_Course_Table+" where "+course_id+" = "+CourseID;
        Cursor cursor = db.rawQuery(Sql,null);
        if(cursor.moveToFirst()) {
            while (cursor.moveToNext()){
                students_ids.add(cursor.getInt(cursor.getColumnIndex(student_id)));
            }
        }
        if(students_ids.size() > 0){
            for (int i = 0 ; i < students_ids.size(); i++){
                data.add(getStudent(students_ids.get(i)));
            }
            return data;
        }
        return null;
    ...
    public ArrayList<StudentModel> getStudents(){
        ...
        SQLiteDatabase db = getReadableDatabase();
        String Sql = "select * from "+StudentTable+" where ";
        Cursor cursor = db.rawQuery(Sql,null);
        while (cursor.moveToNext()){
            Students.add(new StudentModel(
                    cursor.getInt(cursor.getColumnIndex(id)),
                    cursor.getBlob(cursor.getColumnIndex(student_image)),
    ...
    public int AddStudent(StudentModel mStudentModel){
        ...
        try {
            ...
            SQLiteDatabase db = getReadableDatabase();
            String Query = "select id from " + StudentTable + " where "+email+" = ?";
            Cursor cursor = db.rawQuery(Query, new String[]{studentModel.getName()});
            if (cursor.moveToNext()) {
                return StudentID;
            } else {
                ContentValues contentValues = new ContentValues();
                contentValues.put(student_image,mStudentModel.getImageResource());
                contentValues.put(section,studentModel.getSection());
                contentValues.put(department,studentModel.getDepartment());
    ...
    public CourseModel getCourse(int course_id){
        try {
            ...
            while (cursor.moveToNext()) {
                mCourseModel = new CourseModel(course_id, cursor.getString(cursor.getColumnIndex(name)),
    ...
    public void AddCourse(CourseModel courseModel){
        ...
        try {
            ...
            contentValues.put(description,courseModel.getDescription());
